...
validationGenerator = testDataGen.flow_from_directory(os.path.join('Split Dataset', 'Validation'),
                                                      ...
                                                      color_mode='grayscale',
                                                      classes=[
                                                          str(Class) for Class in range(47)],
                                                      class_mode='categorical')
...
callbacks = [ReduceLROnPlateau(monitor='val_loss', factor=0.1,
             ...
             ModelCheckpoint(os.path.join(
                 'Model_5', 'backup_last_model.hdf5')),
             ModelCheckpoint(os.path.join('Model_5', 'best_val_acc.hdf5'),
                             monitor='val_accuracy', mode='max', save_best_only=True),
             ModelCheckpoint(os.path.join('Model_5', 'best_val_loss.hdf5'), monitor='val_loss', mode='min', save_best_only=True)]

model.fit(trainGenerator, epochs=50,
