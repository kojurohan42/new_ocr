...
trainGenerator = trainDataGen.flow_from_directory(os.path.join('Split Dataset', 'Train'),
                                                  ...
                                                  classes = [str(Class) for Class in range(47)],
...
model.add(Conv2D(128, (3, 3), padding = 'same', activation = 'relu', kernel_initializer = 'he_uniform', input_shape = (32, 32, 1)))
...
callbacks = [ReduceLROnPlateau(monitor = 'val_loss', factor = 0.1,
             ...
             EarlyStopping(monitor = 'val_loss', patience = 9, # Patience should be larger than the one in ReduceLROnPlateau
                          min_delta = 1e-5),
             CSVLogger(os.path.join('Model_4', 'training.log'), append = True),
             ModelCheckpoint(os.path.join('Model_4', 'backup_last_model.hdf5')),
             ModelCheckpoint(os.path.join('Model_4', 'best_val_acc.hdf5'), monitor = 'val_accuracy', mode = 'max', save_best_only = True),
             ModelCheckpoint(os.path.join('Model_4', 'best_val_loss.hdf5'), monitor = 'val_loss', mode = 'min', save_best_only = True)]

model.fit(trainGenerator, epochs = 50, validation_data = validationGenerator, callbacks = callbacks)

model = load_model(os.path.join('Model_4', 'best_val_loss.hdf5'))
