...
Validation_Report = pd.DataFrame.from_dict(Dict, orient='index', columns=[
                                           'accuracy_score', 'precision_score', 'recall_score', 'f1_score']).round(4)

Expected_Outcomes = [[['क', 'ल', 'म'], ['प', 'त', 'ल'], ['र', 'व', 'न']],
                     [['क', 'म', 'ल'], ['फ', 'स', 'ल'], ['म', 'ह', 'ल'], ['च', 'म', 'क'], [
                         'ल', 'प', 'क'], ['प', 'ट', 'क'], ['न', 'ह', 'र'], ['प', 'ह', 'र'], ['ल', 'ह', 'र']],
                     [['घ', 'ब', 'न'], ['क', 'च', 'ट'], ['च', 'र', 'म']],
                     [['ज']],
                     [['ह']],
                     [['घ', 'ब', 'न']],
...
y_test = [Predict_Characters.Reversed_Label_Dict[Character]
          for Image in Expected_Outcomes for Word in Image for Character in Word]
Predictions = np.array(
    [Character for Image in Predictions for Word in Image for Character in Word]).T.tolist()
...
Test_Report = pd.DataFrame.from_dict(Dict, orient='index', columns=[
                                     'accuracy_score', 'precision_score', 'recall_score', 'f1_score']).round(4)
